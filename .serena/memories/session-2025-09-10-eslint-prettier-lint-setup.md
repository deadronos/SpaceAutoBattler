...
2. Rewrote/updated `eslint.config.ts` (flat ESM-aware config):
   ...
   - Ensured `parserOptions.project` and `tsconfigRootDir` resolved using fileURLToPath(\_\_filename) for ESM.
...
Session note: Reviewed and re-saved on 2025-09-15 during documentation sweep.
