...
- **Changes:** At the very beginning of the `simulateStep` function, _before_ any physics or AI updates modify `ship.pos`, `ship.orientation`, or `bullet.pos`, add loops to copy the current state to the `prev` properties.
...
- **Changes:**
  ...
  2.  **Calculate Interpolation Factor:**
      ...
      - **Refinement for `timeSinceLastSimStep`:** A more robust way to calculate `timeSinceLastSimStep` would be to pass the `state.time` from the _previous_ `simulateStep` call into the rendering function, or have the rendering function track the `state.time` from the last time it rendered. The key is to know how much time has passed _within the current simulation step_.
