...
- Damage routing summary (relevant bits):
  ...
  - If shields break, remaining damage is reduced by an absolute armor absorption: armorAbs = min(remainingDamage * 0.5, armor * armorEffectiveness).
...
- For each hull we estimate:
  ...
  2. Per-hit armor absorption using armorAbs = min(hit * 0.5, armor * 1.2).
