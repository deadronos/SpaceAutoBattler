...
interface ExplosionConfigEntry {
  ...
  particleCounts: {             // Particle counts by type
    sparks: number;
    plasma: number; 
    smoke: number;
  };
  palette: {                    // Color palette for explosion stages
...
*Future Enhancement:* The configuration structure supports hot-reload capability through:
