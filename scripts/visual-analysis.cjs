...
function analyzeImage(filePath, label) {
  ...
  console.log(`Luminance ratio: ${luminanceRatio.toFixed(2)}× ${luminanceRatio >= 3.3 ? '✓' : '✗'} (target: ≥3.3×)`);
  ...
  console.log(`Filament variance at r=0.45: σ = ${standardDeviation.toFixed(4)} ${standardDeviation >= 0.08 ? '✓' : '✗'} (target: ≥0.08)`);
  ...
  const halo10Check = haloBrightnessRatio >= 0.10;
  ...
  return {
    ...
    passesAllTests: luminanceRatio >= 3.3 && standardDeviation >= 0.08 && halo35Check && halo10Check
  };
}
...
if (beforeResults && afterResults) {
  console.log('\n=== COMPARISON SUMMARY ===');
  console.log(`Luminance ratio: ${beforeResults.luminanceRatio.toFixed(2)}× → ${afterResults.luminanceRatio.toFixed(2)}× (${afterResults.luminanceRatio > beforeResults.luminanceRatio ? '↑' : '↓'})`);
  console.log(`Filament variance: ${beforeResults.filamentVariance.toFixed(4)} → ${afterResults.filamentVariance.toFixed(4)} (${afterResults.filamentVariance > beforeResults.filamentVariance ? '↑' : '↓'})`);
  console.log(`Halo brightness: ${(beforeResults.haloBrightnessRatio * 100).toFixed(1)}% → ${(afterResults.haloBrightnessRatio * 100).toFixed(1)}% (${afterResults.haloBrightnessRatio < beforeResults.haloBrightnessRatio ? '↓' : '↑'})`);
  ...
  console.log(`✓ Controlled halo brightness: ${afterResults.passesHaloUpperTest && afterResults.passesHaloLowerTest ? 'PASS' : 'FAIL'}`);
