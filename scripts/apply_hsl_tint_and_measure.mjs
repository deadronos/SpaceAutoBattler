...
function compareLuminance(base, other){
  if(base.width !== other.width || base.height !== other.height) throw new Error('Size mismatch');
  ...
  for(let i=0;i<bd.length;i+=4){
    ...
    sumBase += lb; sumDiff += Math.abs(lb-lo); n++;
  }
...
(async function(){
  ...
  if(!fs.existsSync(hull)) { console.error('Missing hull image:', hull); process.exit(2); }
  ...
  if(mulP){ console.log('Comparing Multiply -> base...'); console.log(JSON.stringify(compareLuminance(base, mulP), null, 2)); }
  if(flatP){ console.log('Comparing Flat -> base...'); console.log(JSON.stringify(compareLuminance(base, flatP), null, 2)); }
