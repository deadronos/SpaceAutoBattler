...
function calculateRangeStats(ranges: number[]): { min: number; max: number; mean: number; iqr: number } {
...
for (const hull of hulls) {
  ...
  console.log(`${hull.padEnd(10)} | Base: ${baseSpeed.toString().padStart(2)} | Hull: ${newSpeed.toString().padStart(2)} | Type: ${finalSpeed.toString().padStart(2)} | ${bulletType}`);
}
