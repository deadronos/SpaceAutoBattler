...
async function main() {
  ...
  if (!source) { console.error('Source required'); process.exit(1); }
  ...
  if (!(await fileExists(snapshotPath))) {
    ...
    let doc;
    try { doc = JSON.parse(raw); } catch (err) { console.error('Failed to parse snapshot JSON:', err); process.exit(1); }
    doc.meta = doc.meta || {}; doc.meta.updated = new Date().toISOString();
    doc.nodes = doc.nodes || [];
    doc.nodes.push(node);
