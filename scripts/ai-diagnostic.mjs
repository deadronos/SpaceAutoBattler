...
fs.writeFileSync('tmp/ai-diagnostic.log', '');
...
for (const config of configs) {
  ...
  const ships = config.ships.map((spec, idx) => {
    ...
    const ship = {
      id,
      rigidBody: {
        setNextKinematicTranslation: ({ x, y, z }) => position.set(x, y, z),
        setNextKinematicRotation: ({ x, y, z, w }) => rotation.set(x, y, z, w),
    ...
    if (spec.velocity) ship.__harnessVelocity = new Vector3(...spec.velocity);
    return ship;
  });

  const state = {
    ai: { tickIndex: 1, assignments: { escorts: new Map() } },
    blackboard: {
      tickIndex: 1,
  ...
  for (const ship of ships) {
    ...
    const escort = escortAssignment
      ? (ships.find((s) => s.id === escortAssignment.vipId) ?? null)
      : null;

    const attack = __aiTestHooks.scoreAttackIntent(
      ship,
      profile,
      primary,
    ...
    const kite = __aiTestHooks.scoreKiteIntent(
      ship,
      profile,
      primary,
      state.blackboard.teamPosture[ship.ship.team],
    ...
    const intercept = primary
      ...
      : 0;
    const reposition = __aiTestHooks.scoreRepositionIntent(
      state,
      ship,
      profile,
      primary,
    ...
    const regroup = __aiTestHooks.scoreRegroupIntent(
      state,
      ship,
      profile,
      state.blackboard.teamPosture[ship.ship.team],
    ...
    const flee = __aiTestHooks.scoreFleeIntent(
      ship,
      profile,
      primary,
      state.blackboard.teamPosture[ship.ship.team],
    ...
    const chosen = __aiTestHooks.selectIntent(
      state,
      ship,
      ai,
      profile,
