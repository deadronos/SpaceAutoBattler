...
- 2026-06-06: Memory bank source audit (compare against current `src/**`) — fixed duplicate line in `core-ships.md` (turret registration copy-paste error), removed broken `guides/AI_DEPRECATION_GUIDE.md` and `guides/TEST_HARNESS_PATTERNS.md` references from `core-aiProfiles.md` and `core-aiScenarioHarness.md` (the `guides/` folder was never created), and added four new memory entries: `core-cappedBuffer.md` (TASK106/DESIGN055), `core-simulationBridge.md` (TASK157 worker simulation), `core-projectileSystems.md` (TASK103/104 projectile folder split), and `core-steering.md` (TASK161 allocation-free steering helpers). Refreshed `memory/_index.md` to link the new entries and re-grouped the index by topic; updated the "Audited on" line. No code changes were made; documents updated only. Validation: read-only file comparison against the current source.
