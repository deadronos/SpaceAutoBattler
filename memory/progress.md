...
- 2026-06-06 (refactor): Split 3 monolithic files + extract cross-cutting helpers — `ships.ts` → `turretFactory.ts`, `damage.ts` → `damageMath.ts` + `damageApplication.ts`, `systems.ts` → `pipeline.ts` (declarative ordering). Extracted `tempVectors.ts` (shared scratch pool) and `physicsBodyManager.ts` (body lifecycle). Wired up consumers: `steering.ts`, `aiming.ts`, `turrets.ts` now import from `tempVectors.ts`; `entityLifecycle.ts` uses `destroyBody`. Added `pipeline.spec.ts` (8 tests) and `turret-factory.spec.ts` (5 tests). Validation: typecheck clean, 923 tests pass. PR #591.
- 2026-06-06 (docs): Memory bank source audit — fixed duplicate line in `core-ships.md` (turret registration copy-paste error), removed broken `guides/TEST_HARNESS_PATTERNS.md` references, and added four new memory entries (`core-cappedBuffer.md`, `core-simulationBridge.md`, `core-projectileSystems.md`, `core-steering.md`). Refreshed `memory/_index.md`. No code changes; documents updated only.
