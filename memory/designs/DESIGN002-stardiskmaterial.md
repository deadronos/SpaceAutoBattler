...
- Implement the new files and keep `src/renderer/starDiskMaterial.ts` as a re-export (index) file that imports and re-exports the public functions while logging that the file is now a proxy.
- Replace internal import paths step-by-step. Run typecheck and tests after each step.
