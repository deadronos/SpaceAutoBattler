...
High-level layering
- Physics root (immutable for visuals): physicsRootRef — receives interpolated physics pose (interpolated between last & current tick using sim.alpha). This is the pose that colliders and gameplay logic reflect.
...
interface RendererConfig { visualSmoothing?: { enableShipVisualSmoothing: boolean } }
```
