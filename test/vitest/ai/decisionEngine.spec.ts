...
import { scorePursue, scoreEvade, scoreRoam, chooseBestIntent } from '../../../src/core/ai/decisionEngine.js';
...
describe('decisionEngine scoring helpers', () => {
  ...
  it('scorePursue increases with closer distance and aggressiveness', () => {
    const far = scorePursue({ distanceToEnemy: 1000, preferredRange: 500, settings, personality, isScout: false, teamUnderAlarm: false });
    const medium = scorePursue({ distanceToEnemy: 300, preferredRange: 500, settings, personality, isScout: false, teamUnderAlarm: false });
    const close = scorePursue({ distanceToEnemy: 100, preferredRange: 500, settings, personality, isScout: false, teamUnderAlarm: false });
    expect(medium).toBeGreaterThan(far);
    expect(close).toBeGreaterThan(medium);
  });

  it('scoreEvade spikes with damage and proximity', () => {
    const low = scoreEvade({ distanceToThreat: 2000, recentDamage: 0, damageEvadeThreshold: 50, withinRecentDamageWindow: false, settings });
    const withDamage = scoreEvade({ distanceToThreat: 2000, recentDamage: 60, damageEvadeThreshold: 50, withinRecentDamageWindow: true, settings });
    const proximityThreshold = settings.minimumSafeDistance * settings.closeRangeMultiplier;
    const closeThreat = scoreEvade({ distanceToThreat: Math.max(0.1, proximityThreshold - 1), recentDamage: 0, damageEvadeThreshold: 50, withinRecentDamageWindow: false, settings });
