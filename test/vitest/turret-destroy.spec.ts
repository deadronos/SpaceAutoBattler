...
function makeStateStub(): GameState {
  ...
  const world = {
    entities,
    createEntity(obj: any) { entities.push(obj); return obj; },
    destroyEntity(obj: any) { const i = entities.indexOf(obj); if (i >= 0) entities.splice(i, 1); }
  } as any;

  const rapierStub = {
    RigidBodyDesc: { kinematicPositionBased: () => ({ setTranslation() { return this; }, setRotation() { return this; } }) },
...
function makeShipAndTurret(state: GameState): { ship: ShipEntity; turret: TurretEntity } {
  ...
  const ship: ShipEntity = {
    ...
    ship: { team: 'blue', hull: 'corvette' as any, hp: 10, maxHp: 10, shield: 0, maxShield: 0, cooldown: 0, fireRate: 1, damage: 1, projectileSpeed: 1, range: 1, speed: 0, bulletType: '' },
  ...
  const turret: TurretEntity = {
    ...
    transform: { position: new Vector3(0,0,0), rotation: new Quaternion(), scale: 1 },
    turret: { parent: ship, offset: new Vector3(0,0,0), damage: 1, fireRate: 1, projectileSpeed: 1, range: 1, bulletType: '', cooldown: 0, index: 0, yaw:0, pitch:0, priority: 'any' }
  } as TurretEntity;
