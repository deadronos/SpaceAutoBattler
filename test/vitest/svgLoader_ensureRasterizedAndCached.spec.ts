...
vi.mock('../../src/assets/svgRenderer', () => {
  ...
  const fakeCanvas = { width: 64, height: 64, __fake: true } as unknown as HTMLCanvasElement;
  return {
...
describe('ensureRasterizedAndCached', () => {
  ...
  it('triggers async rasterization and populates cache when cache miss', async () => {
    ...
    const getCanvasStub = (svgRenderer as any).getCanvas ?? (svgRenderer as any).default?.getCanvas;
    expect(getCanvasStub()).toBeNull();

    // Support both named exports and a default export shape
    const rasterizeStub = (svgRenderer as any).rasterizeSvgWithTeamColors ?? (svgRenderer as any).default?.rasterizeSvgWithTeamColors;
    const cacheSpy = (svgRenderer as any).cacheCanvasForAsset ?? (svgRenderer as any).default?.cacheCanvasForAsset;

  // Act: call ensureRasterizedAndCached and capture the returned promise
  const resultPromise = (svgLoader as any).ensureRasterizedAndCached(sampleSvg, mapping, outW, outH, { assetKey: 'test-asset' });
