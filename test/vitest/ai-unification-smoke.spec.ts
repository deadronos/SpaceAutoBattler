...
describe('AI Unification Smoke Test', () => {
  it(
    'should handle 10-20 ships per side with consistent behavior for 10 seconds',
    { timeout: 60000 },
    () => {
      ...
      const shipsWithAIState = state.ships.filter(
        (s) =>
          (s as unknown as Record<string, unknown>)['aiState'] !== undefined ||
          (s.turrets && s.turrets.length > 0),
      ).length;
      ...
      for (const ship of state.ships) {
        ...
        expect(ship.pos.x).toBeGreaterThanOrEqual(
          -Math.max(400, TEST_DEFAULTS.simBounds.width * 0.1),
        );
        expect(ship.pos.x).toBeLessThanOrEqual(
          state.simConfig.simBounds.width + Math.max(500, TEST_DEFAULTS.simBounds.width * 0.25),
        );
        expect(ship.pos.y).toBeGreaterThanOrEqual(
          -Math.max(200, TEST_DEFAULTS.simBounds.height * 0.05),
        );
        expect(ship.pos.y).toBeLessThanOrEqual(
          state.simConfig.simBounds.height + Math.max(200, TEST_DEFAULTS.simBounds.height * 0.1),
        );
      ...
      const shipsWithTargets = state.ships.filter(
        (s) =>
          (s.targetId !== null && s.targetId !== undefined) ||
          (s as unknown as Record<string, unknown>)['__aiAssignedTarget'] !== undefined ||
          (s.turrets && s.turrets.length > 0),
      ).length;
  ...
  it(
    'should produce consistent behavior between different AI configurations',
    () => {
      ...
      if (
        minimalState.spatialGrid &&
        minimalState.behaviorConfig?.globalSettings.enableSpatialIndex
      ) {
        minimalState.spatialGrid.rebuild(
          minimalState.ships.map((s) => ({ id: s.id, pos: s.pos, radius: 16, team: s.team })),
        );
      }
      ...
      if (fullState.spatialGrid && fullState.behaviorConfig?.globalSettings.enableSpatialIndex) {
        fullState.spatialGrid.rebuild(
          fullState.ships.map((s) => ({ id: s.id, pos: s.pos, radius: 16, team: s.team })),
        );
      }
      ...
      for (const ship of minimalState.ships.concat(fullState.ships)) {
        ...
        expect(ship.pos.x).toBeLessThanOrEqual(
          minimalState.simConfig.simBounds.width + 1 + TEST_DEFAULTS.simBounds.width * 0.01,
        );
        expect(ship.pos.y).toBeLessThanOrEqual(
          minimalState.simConfig.simBounds.height + 1 + TEST_DEFAULTS.simBounds.height * 0.01,
        );
        expect(ship.pos.z).toBeLessThanOrEqual(
          minimalState.simConfig.simBounds.depth + 1 + TEST_DEFAULTS.simBounds.depth * 0.01,
        );
      }
    },
    { timeout: 20000 },
  ...
  it('should steer ships inward when spawned at edges', () => {
    ...
    for (let i = 0; i < frames; i++) {
      ...
      if (s.spatialGrid && s.behaviorConfig?.globalSettings.enableSpatialIndex) {
        s.spatialGrid.rebuild(
          s.ships.map((sh) => ({ id: sh.id, pos: sh.pos, radius: 16, team: sh.team })),
        );
      }
    }
