...
function makeStateStub(): GameState {
  ...
  const rapierStub = {
    RigidBodyDesc: {
      kinematicPositionBased: () => {
        ...
        d.setTranslation = function (x: number, y: number, z: number) { this._pos = { x, y, z }; return this; };
        d.setRotation = function (r: { x: number; y: number; z: number; w: number }) { this._rot = r; return this; };
        return d;
      }
    },
    ColliderDesc: {
  ...
  return {
    ...
    nextEntityId: 1,
    time: 0,
    queries: { ships: { entities: [] }, projectiles: { entities: [] }, turrets: { entities: [] } } as any,
    rng: { next: () => 0.5 } as any,
    paused: false,
    timeScale: 1
  } as GameState;
...
function makeShipWithTurret(team: 'red'|'blue', pos: Vector3, turret: Omit<TurretState, 'cooldown'> & { cooldown?: number }): ShipEntity {
...
describe('Turret system', () => {
  ...
  const enemyShip: ShipEntity = {
      ...
      ship: {
        team: 'red', hull: 'fighter' as any,
        hp: 10, maxHp: 10, shield: 0, maxShield: 0,
         cooldown: 999, fireRate: 1, damage: 2, projectileSpeed: 10, range: 10, speed: 0, bulletType: 'bullet:ion'
      },
  ...
  const expectedWorld = new Vector3(1,0,0).applyQuaternion(friendly.transform.rotation).add(friendly.transform.position);
