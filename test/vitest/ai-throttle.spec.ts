...
function makeState(): GameState {
  ...
  const rng = { seed: 'TEST', next: () => 0.5, int: (a: number, _b: number)=>a, pick: <T>(arr: readonly T[])=>arr[0] } as any;
...
function addShip(state: GameState, team: 'red'|'blue', pos = {x:0,y:0,z:0}): Ship {
  ...
  try {
    if ((state as any).spatialGrid && (state as any).spatialGrid.rebuild) {
      (state as any).spatialGrid.rebuild(state.ships.map(s => ({ id: s.id, pos: s.pos, radius: 16, team: s.team })));
    }
...
describe('AI throttling', () => {
  ...
  expect(red.targetId === closer.id || red.targetId === initialTarget || red.targetId === null).toBe(true);
  });
  ...
  expect((t.aiState?.targetId ?? null) === first || (t.aiState?.targetId ?? null) === null).toBe(true);
