...
const fakeCanvas = (() => { const c = document.createElement('canvas'); c.width = outW; c.height = outH; return c; })() as HTMLCanvasElement;
...
describe('svgLoader cache hit and fallback behavior', () => {
  it('getCachedHullCanvasSync returns cached canvas when svgRenderer.getCanvas has an entry', async () => {
    ...
    expect((res as HTMLCanvasElement).width).toBe(outW);

    // Ensure the upstream rasterizer was NOT invoked when getCanvas returned a cached canvas
    expect((svgRenderer as any).rasterizeSvgWithTeamColors).not.toHaveBeenCalled();
  ...
  const rasterizeAsyncSpy = vi.spyOn(svgLoader, 'rasterizeSvgToCanvasAsync').mockResolvedValue(fakeCanvas as any);
  ...
  try { expect(cacheSpy).toHaveBeenCalled(); } catch (e) { /* ok - depending on path, cache may be optional */ }
