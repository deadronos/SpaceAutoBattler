...
vi.mock('../../src/assets/svgRenderer', () => {
  ...
  const cachedCanvas = { width: 64, height: 64, __cached: true } as unknown as HTMLCanvasElement;
  return {
...
describe('ensureRasterizedAndCached (cache hit)', () => {
  ...
  it('returns cached canvas and does not call rasterizer', async () => {
    ...
    expect((cached as any).__cached).toBe(true);

    // Act
    const resolved = await (svgLoader as any).ensureRasterizedAndCached(sampleSvg, mapping, 64, 64, { assetKey: 'test-asset' });
