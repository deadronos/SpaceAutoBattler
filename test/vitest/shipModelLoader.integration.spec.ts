...
describe('shipModelLoader threePrototypes extraction', () => {
  beforeEach(() => {
    ...
    (
      SHIP_MODEL_MAP as unknown as Record<
        string,
        {
          file: string;
          scale?: number;
  ...
  it('extracts geometries and materials, honoring nested transforms and multi-material meshes', async () => {
    ...
    (loadGLTF as unknown as ReturnType<typeof vi.fn>).mockImplementation(
      async (_state: unknown, _url: string) => {
    ...
    const geoms =
      stored && stored.threePrototypes
        ? (stored.threePrototypes.geometries as THREE.BufferGeometry[])
        : [];
    const mats =
      stored && stored.threePrototypes
        ? (stored.threePrototypes.materials as THREE.Material[])
        : [];
    ...
    let allocated = false;
    try {
      allocated = shipInstancer.allocate(12345, 'fighter', 'red', state);
    } catch {
      allocated = false;
    }
