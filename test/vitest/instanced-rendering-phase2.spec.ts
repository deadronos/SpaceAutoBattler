...
import { InstancedRenderer, InstanceData, BatchRenderCall } from '../../src/webgl/instancedRenderer';
import { TextureAtlas, AtlasManager, AtlasEntry } from '../../src/assets/textureAtlas';
...
class MockWebGLContext {
  ...
  getShaderParameter(shader: any, pname: number): boolean {
  ...
  getProgramParameter(program: any, pname: number): boolean {
  ...
  deleteShader(shader: any): void {
    if (shader?.id) this.mockObjects.delete(shader.id);
  }
  
  deleteProgram(program: any): void {
    if (program?.id) this.mockObjects.delete(program.id);
  }
  
  deleteBuffer(buffer: any): void {
    if (buffer?.id) this.mockObjects.delete(buffer.id);
  }
  
  deleteTexture(texture: any): void {
    if (texture?.id) this.mockObjects.delete(texture.id);
  }
  
  deleteVertexArray(vao: any): void {
    if (vao?.id) this.mockObjects.delete(vao.id);
  }
...
function createMockCanvas(width = 64, height = 64): HTMLCanvasElement {
...
describe('InstancedRenderer', () => {
  ...
  beforeEach(() => {
    ...
    renderer = new InstancedRenderer(mockGl as any);
  });
...
describe('TextureAtlas', () => {
  ...
  beforeEach(() => {
    mockGl = new MockWebGLContext();
    atlas = new TextureAtlas(mockGl as any, {
      maxAtlasSize: 512,
      padding: 2,
      powerOfTwo: true
    });
...
describe('AtlasManager', () => {
  ...
  beforeEach(() => {
    mockGl = new MockWebGLContext();
    manager = new AtlasManager(mockGl as any, {
      maxAtlasSize: 256,
      padding: 1
    });
  });
