...
function getPixel(canvas: HTMLCanvasElement, x: number, y: number) {
...
describe('UI bars orientation', () => {
  it('HP/shield bars do not rotate with ship angle', () => {
    ...
    const bg = (renderer as any).bufferCtx;
    if (bg) {
      bg.save();
    ...
    const sample = () => {
      (renderer as any).renderState(state);
  ...
  const r = (ship as any).radius || 12;
  const sy = Math.round(ship.y - r - 12);
      return getPixel((renderer as any).canvas, sx, sy);
    };
    ...
    const isBg = (p: number[]) => p[0] === 0 && p[1] === 0 && p[2] === 0 && p[3] === 0;
