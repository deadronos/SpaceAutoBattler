...
import svgRenderer from '../../src/assets/svgRenderer';
...
describe('SVG tinting and rasterization', () => {
  ...
  it('CanvasRenderer creates and caches a tinted hull canvas keyed by shipType::color', async () => {
    ...
    (AssetsConfig as any).svgAssets = { ...(AssetsConfig as any).svgAssets, [shipType]: svg };
  ...
  const teamColor = '#112233';
    const state: any = { ships: [{ id: 1, type: shipType, x: 100, y: 100, radius: 12, team: 'red' }], flashes: [] };
  // Ensure TeamsConfig has an entry with this color so renderer resolves it
  try { (TeamsConfig as any).teams.red.color = teamColor; } catch (e) {}
    ...
    if (tctx) { tctx.fillStyle = teamColor; tctx.fillRect(0,0,32,32); }
    const key = `${shipType}::${teamColor}`;
    // Use the test helper to insert into pool
    try {
      (renderer as any)._testSetTintedCanvas(key, tc);
    } catch (e) {}
    // Now the Map-like facade should report the key present
    const has = (renderer as any)._tintedHullCache && (renderer as any)._tintedHullCache.has(key);
