...
import type { PoolEntry } from '../../../src/types/pool';
import PoolManager, { makePoolEntry, ensureEntryForKey, clearEntryFreeList } from '../../../src/pools/PoolManager';

describe('PoolManager basic behavior', () => {
  it('makePoolEntry returns sensible defaults', () => {
    const e = makePoolEntry<number>();
  ...
  it('ensureEntryForKey creates and reuses an entry in a Map', () => {
    const map = new Map<string, PoolEntry<number>>();
  ...
  it('clearEntryFreeList calls sync disposer and clears freeList', async () => {
    ...
    const entry = makePoolEntry<number>({ disposer: (n) => { disposed.push(n); /* void */ } });
  ...
  it('clearEntryFreeList supports async disposers', async () => {
    const disposed: number[] = [];
    const entry = makePoolEntry<number>({ disposer: async (n) => { await new Promise(r => setTimeout(r, 1)); disposed.push(n); } });
