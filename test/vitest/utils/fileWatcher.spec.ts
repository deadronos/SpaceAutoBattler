...
describe('FileWatcher', () => {
  ...
  const watcher = getFileWatcher();
    ...
    let called: any = null;
    watcher.watch('assets/foo.svg', (path: string, change: any) => { called = { path, change }; });
  ...
  it('handles missing file (HEAD returns non-ok) gracefully', async () => {
    ...
    (globalThis as any).fetch = vi.fn().mockResolvedValue({ ok: false, headers: { get: () => null } });
  ...
  const watcher = getFileWatcher();
    ...
    let called: any = null;
    watcher.watch('http://example.com/missing.svg', (path: string, change: any) => { called = { path, change }; });
