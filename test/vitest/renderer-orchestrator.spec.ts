...
import {
  ...
  setCameraDistance,
  getCameraDistance,
} from '../../src/renderer/cameraManager.js';
  ...
  describe('Module Integration', () => {
    ...
    it('should coordinate mesh factory and synchronizer', () => {
      ...
      const ship = createMockShip({
        ...
        turrets: (fighterCfg.turrets || []).map((t: any, idx: number) => ({
          id: `${t.id}-${idx}`,
          cooldownLeft: 0,
        })),
      });
      ...
      const shipMesh = meshFactory.createShipMesh(
        ship,
        state,
        groups.shipsGroup,
        syncState.shipMeshes,
      );
      const defaultDamage =
      ...
      expect(() =>
        shieldEffect.updateShieldEffect(
          ship,
          shieldEffectMesh,
          Date.now() / 1000,
          shieldEffectState,
