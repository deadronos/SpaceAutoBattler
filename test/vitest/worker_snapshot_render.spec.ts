...
describe('Worker snapshot -> renderer integration', () => {
  it('renders worker snapshots immediately', () => {
    ...
    const calls: any[] = [];
    const fakeRenderer = {
      renderState: (s: any) => { calls.push(s); },
    };

    const gm = createGameManager({ renderer: fakeRenderer, useWorker: true, seed: 1234, createSimWorker: fakeFactory });

    // Build a snapshot-like state and call the worker's snapshot handler
    const state = makeInitialState();
    state.ships.push(createShip(undefined, 12, 34, 'red'));
    state.bullets.push({ id: 1, x: 100, y: 200, vx: 0, vy: 0, team: 'red', damage: 1, ttl: 1 } as any);
