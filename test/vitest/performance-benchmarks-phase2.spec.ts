...
class BenchmarkWebGLContext {
  ...
  private performanceNow = mockPerformanceNow();
  
  // Track method call counts for performance analysis
  private trackCall(method: string) {
...
function generateInstanceData(count: number): InstanceData[] {
...
describe('Performance Benchmarks - Phase 2 Instanced Rendering', () => {
  let mockGl: BenchmarkWebGLContext;
  ...
  beforeEach(() => {
    ...
    atlasManager = new AtlasManager(mockGl as any, {
      maxAtlasSize: 1024,
      padding: 1
    });
  });
  ...
  it('should measure instanced rendering performance vs traditional draws', () => {
    ...
    const results: Record<number, { instanced: BenchmarkResult; traditional: BenchmarkResult }> = {};
    ...
    for (const count of instanceCounts) {
      ...
      const instancedResult = benchmark(`Instanced Rendering (${count} instances)`, () => {
        const batch: BatchRenderCall = { texture, instances, blendMode: 'normal' };
  ...
  it('should measure batch grouping efficiency', () => {
    ...
    const result = benchmark('Batch Grouping and Rendering', () => {
      ...
      const batches = new Map<any, InstanceData[]>();
...
describe('Scalability Analysis', () => {
  it('should analyze performance scaling characteristics', () => {
    ...
    const renderer = new InstancedRenderer(mockGl as any);
    renderer.init();
    
    const scalingPoints = [10, 50, 100, 500, 1000, 2000, 5000];
    const results: Array<{ count: number; timeMs: number; efficiency: number }> = [];
