...
describe('shipMap sync', () => {
  ...
  if (!ship) return; // defensive - satisfy types
  expect((state as any).shipMap).toBeDefined();
  expect((state as any).shipMap.has(ship.id)).toBe(true);
  ...
  const found = (state.ships || []).find((s: any) => s && s.id === ship.id);
    expect(found).toBeUndefined();
  expect((state as any).shipMap.has(ship.id)).toBe(false);
  });
});
