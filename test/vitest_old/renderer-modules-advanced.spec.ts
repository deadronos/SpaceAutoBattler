...
import {
  createShieldEffect,
  updateShieldEffect,
  createShieldEffectState,
} from '../../src/renderer/effects/shieldEffect.js';
import {
  shieldVertexShader,
  createShieldFragmentShader,
} from '../../src/renderer/effects/shieldShaders.js';
import {
  createSynchronizerState,
  createSynchronizerGroups,
  syncEntities,
  updateTransforms,
} from '../../src/renderer/synchronizer.js';
...
describe('Additional Renderer Modules', () => {
  describe('Shield Effect Module', () => {
    ...
    it('should update shield effects without errors', () => {
      ...
      const ship = createMockShip({
        ...
        turrets: (frigateCfg.turrets || []).map((t: any, idx: number) => ({
          id: `${t.id}-${idx}`,
          cooldownLeft: 0,
        })),
    ...
    it('should update transforms with mocked factories', () => {
      ...
      state.ships.push(
        createMockShip({
          ...
          turrets: (corvetteCfg.turrets || []).map((t: any, idx: number) => ({
            id: `${t.id}-${idx}`,
            cooldownLeft: 0,
          })),
        }),
      ...
      const mockShieldEffect = {
        createShieldEffect: (
          _ship: ShipType,
          _state: import('../../src/renderer/effects/shieldEffect.js').ShieldEffectState,
        ) => new THREE.Object3D(),
        updateShieldEffect: () => {},
      ...
      expect(() =>
        updateTransforms(state, syncState, mockMeshFactory, mockShieldEffect, shieldEffectState),
      ).not.toThrow();
  ...
  describe('Overlay Module', () => {
    ...
    it('should update billboard overlays', () => {
      ...
      expect(() =>
        updateBillboardOverlays(healthBarMeshes, cameraState, factoryState, overlayState),
      ).not.toThrow();
    });
  });
