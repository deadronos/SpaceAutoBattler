...
import {
  ...
  findNearbyEnemies,
  findNearbyFriends,
  getNearbySeparationShipsLinear,
...
function makeState(ships: any[], useSpatial = false) {
  ...
  const state: any = {
    ...
    simConfig: {
      ...
      boundaryBehavior: { ships: 'wrap', bullets: 'wrap' },
    },
  };
...
describe('searchUtils', () => {
  ...
  it('findNearestEnemy works with linear fallback', () => {
    const ships = [
      makeShip(1, 'red', 0, 0),
      makeShip(2, 'blue', 10, 0),
      makeShip(3, 'blue', 30, 0),
    ];
  ...
  it('findNearestEnemy prefers spatial index when enabled', () => {
    const ships = [
      makeShip(1, 'red', 0, 0),
      makeShip(2, 'blue', 300, 0),
      makeShip(3, 'blue', 10, 0),
    ];
