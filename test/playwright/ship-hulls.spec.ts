...
async function saveDebugArtifacts(
  ...
  // Save scene summary
  fs.writeFileSync(
    path.join(debugPath, 'scene-summary.json'),
  ...
  for (const hullId of hullsToTest) {
    test.describe(`Hull: ${hullId}`, () => {
      ...
      test('should have expected scene structure', async ({ page }, testInfo) => {
        ...
        try {
          ...
          for (const material of summary.materials) {
            ...
            expect(['MeshStandardMaterial', 'MeshBasicMaterial', 'MeshPhysicalMaterial', 'ShaderMaterial']).toContain(material.type);
          }
        } catch (error) {
      ...
      test.skip('should render shield when enabled', async ({ page }) => {
        ...
        const hasShieldMesh = summary.meshes.some(m => 
          m.name.toLowerCase().includes('shield')
        );
        const hasShieldUniform = typeof summary.uniforms.shieldAlpha === 'number' && summary.uniforms.shieldAlpha > 0;
      ...
      test.skip('should render engine glow when enabled', async ({ page }) => {
        ...
        const hasEngineGlow = summary.materials.some(m => 
          (m.name.toLowerCase().includes('engine') || 
