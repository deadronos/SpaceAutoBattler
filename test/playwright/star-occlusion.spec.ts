...
test.describe('Star occlusion', () => {
  test('a planet should occlude the star when camera rotated into alignment', async ({ page }) => {
    ...
    const probeForDarkPixel = async (pxX: number, pxY: number, radius = 120, step = 6) => {
      return await page.evaluate(async (args: { cx: number; cy: number; r: number; s: number }) => {
         try {
          ...
          // Luminance helper
          const lum = (rr: number, gg: number, bb: number) => 0.2126 * rr + 0.7152 * gg + 0.0722 * bb;
 
          for (let dx = -r; dx <= r; dx += s) {
            for (let dy = -r; dy <= r; dy += s) {
               const x = deviceX + dx;
               const y = deviceY + dy;
               if (x < 0 || y < 0 || x >= (canvas as HTMLCanvasElement).width || y >= (canvas as HTMLCanvasElement).height) continue;
               try {
                 gl.readPixels(x, y, 1, 1, (gl as any).RGBA || 0x1908, (gl as any).UNSIGNED_BYTE || 0x1401, buffer);
               } catch {
                 continue;
               }
    ...
    for (const deg of rotationCandidates) {
      ...
      await page.evaluate((d) => { try { (window as any).__copilot_rotateCameraDeltaDeg = d; } catch { } }, deg);
