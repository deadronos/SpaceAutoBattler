...
function distUrl(baseURL: string | undefined) {
...
test('visual inspect hulls - destroyer/frigate/corvette/carrier', async ({ page }) => {
    ...
    try {
      ...
      for (const pf of DIAGNOSTIC_PREFIXES) {
        if (text.indexOf(pf) !== -1) {
          ...
          try { console.log('[browser]', msg.type(), text); } catch (e) {}
          break;
        }
      }
    } catch (e) {}
  ...
  const networkResponses: Array<any> = [];
  ...
  page.on('requestfailed', (rf) => {
    try {
      let f: any = undefined;
      try { f = (typeof (rf as any).failure === 'function') ? (rf as any).failure() : undefined; } catch (ee) { f = undefined; }
      networkFailures.push({ url: rf.url(), failure: f && (f.errorText || f.message) ? (f.errorText || f.message) : String(f) });
    } catch (e) {}
  });

  const url = distUrl(test.info().project.use?.baseURL as any);
  ...
  await page.addInitScript(() => { (window as any).__AWAIT_SVG_PREWARM = true; });
  ...
  const stepFlow = async (type: string) => {
    ...
    await page.evaluate((t) => {
      ...
      if (sel) (sel as HTMLSelectElement).value = t;
    ...
    if (addRed) {
      ...
      if (addBtn) await addBtn.click().catch(() => null);
    }
    ...
    try {
      await page.evaluate(async (t) => {
        try {
          // @ts-ignore
          const api = (window as any).__SpaceAutoBattler_svgRenderer;
    ...
    const click = async (selectors: string[]) => {
      for (const sel of selectors) {
        const el = await page.$(sel);
        if (el) { await el.click().catch(() => null); return true; }
      }
      return false;
    };

    const started = await click(['text=Start', 'text=Play', 'text=Run', 'button#start', 'button#play']);
    if (!started) console.warn('Start button not found for type', type);
    ...
    const shipsOfType = await page.evaluate((t) => {
      try {
        ...
        if (!s || !Array.isArray(s.ships)) return null;
        return s.ships.filter((sh: any) => sh.type === t).map((sh: any) => ({ id: sh.id, x: sh.x, y: sh.y, angle: sh.angle, type: sh.type }));
      } catch (e) { return null; }
    }, type).catch(() => null as any);
  ...
  try {
    const assetConfig = await page.evaluate(() => {
      // @ts-ignore
      const cfg = (window as any).AssetsConfig || (window as any).assetsConfig || null;
      try {
        if (!cfg || !cfg.svgAssets) return null;
        const entries: Record<string, string> = {};
        for (const k of Object.keys(cfg.svgAssets)) entries[k] = cfg.svgAssets[k];
        return entries;
      } catch (e) { return null; }
    });

    const assetDiag = { assetConfig: assetConfig || null, networkRequests, networkResponses, networkFailures };
    try { fs.writeFileSync(path.join(outDir, `asset-fetch-diag-${Date.now()}.json`), JSON.stringify(assetDiag, null, 2)); } catch (e) {}
  ...
  if (!clicked) console.warn('Start button not found; simulation may remain paused');
  ...
  try { fs.writeFileSync(outJson, JSON.stringify({ results, afterScreenshot }, null, 2)); } catch (e) { console.warn('write json failed', e); }
