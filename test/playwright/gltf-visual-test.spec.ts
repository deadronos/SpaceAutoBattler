...
test.describe('GLTF Mesh Rendering Visual Test', () => {
  ...
  test('should load GLTF assets successfully', async ({ page }) => {
    ...
    const fighterResponse = await page.request.get('http://localhost:8081/src/config/assets/gltf/fighter.glb');
    expect(fighterResponse.status()).toBe(200);
    expect(fighterResponse.headers()['content-type']).toContain('model/gltf-binary');
    
    const corvette = await page.request.get('http://localhost:8081/src/config/assets/gltf/corvette.glb');
    expect(corvette.status()).toBe(200);
    
    const frigate = await page.request.get('http://localhost:8081/src/config/assets/gltf/frigate.glb');
    expect(frigate.status()).toBe(200);
    
    const destroyer = await page.request.get('http://localhost:8081/src/config/assets/gltf/destroyer.glb');
    expect(destroyer.status()).toBe(200);
    
    const carrier = await page.request.get('http://localhost:8081/src/config/assets/gltf/carrier.glb');
