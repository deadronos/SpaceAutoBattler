...
describe('CELESTIAL_ENVIRONMENT planet textures', () => {
  it('references only registered planet texture keys', () => {
    for (const planet of CELESTIAL_ENVIRONMENT.planets) {
      expect(Object.prototype.hasOwnProperty.call(PLANET_TEXTURE_PATHS, planet.textureKey)).toBe(true);
      expect(Object.prototype.hasOwnProperty.call(PLANET_LOWRES_TEXTURE_PATHS, planet.textureKey)).toBe(true);
    }
  });
});
