...
try {
  (self as unknown as { postMessage?: (m: unknown) => void }).postMessage?.({
    type: 'worker-checkpoint',
    name: 'module-eval-start',
  });
} catch {
  /* ignore */
}
...
try {
  (self as unknown as { postMessage?: (m: unknown) => void }).postMessage?.({
    type: 'worker-checkpoint',
    name: 'message-listener-attached',
  });
} catch {
  /* ignore */
}
...
self.addEventListener('error', (e: ErrorEvent) => {
  try {
    ...
    try {
      console.debug('[svgRasterWorker.impl] Error while reporting worker error');
    } catch {
      /* swallow */
    }
  }
...
self.addEventListener('unhandledrejection', (e: PromiseRejectionEvent) => {
  try {
    ...
    try {
      console.debug('[svgRasterWorker.impl] Error while reporting unhandled rejection');
    } catch {
      /* swallow */
    }
  }
...
self.addEventListener('messageerror', (e: MessageEvent) => {
  try {
    ...
    try {
      console.debug('[svgRasterWorker.impl] Error while reporting messageerror');
    } catch {
      /* swallow */
    }
  }
