...
export function calculateEscapeScore(
  ...
  if (targetPos.x > bounds.width - m)
    score -= (targetPos.x - (bounds.width - m)) * settings.evadeBoundaryPenaltyWeight;
  if (targetPos.y < m) score -= (m - targetPos.y) * settings.evadeBoundaryPenaltyWeight;
  if (targetPos.y > bounds.height - m)
    score -= (targetPos.y - (bounds.height - m)) * settings.evadeBoundaryPenaltyWeight;
  if (targetPos.z < m) score -= (m - targetPos.z) * settings.evadeBoundaryPenaltyWeight;
  if (targetPos.z > bounds.depth - m)
    score -= (targetPos.z - (bounds.depth - m)) * settings.evadeBoundaryPenaltyWeight;
...
export function moveTowards(
  ...
  if (distanceSq <= thresholdSq && !ignoreCloseEnough) {
    if (DEBUG_AI) {
      ...
      console.error(
        `AI-DEBUG moveTowards early-return distance=${distance.toFixed(3)} threshold=${settings.movementCloseEnoughThreshold} ignoreCloseEnough=${!!ignoreCloseEnough}`,
      );
    }
    return;
  }
  ...
  if (DEBUG_AI) {
    const distance = sqrt(distanceSq);
    console.error(
      `AI-DEBUG moveTowards ship=${ship.id} distance=${distance.toFixed(6)} moveSpeed=${moveSpeed} accel=${accel.toFixed(6)} dt=${dt.toFixed(6)} forward=${forward.x.toFixed(6)},${forward.y.toFixed(6)},${forward.z.toFixed(6)}`,
    );
  }
  ...
  if (DEBUG_AI)
    console.error(
      `AI-DEBUG moveTowards deltaVel pre=${deltaX.toFixed(9)},${deltaY.toFixed(9)},${deltaZ.toFixed(9)} preVel=${ship.vel.x.toFixed(9)},${ship.vel.y.toFixed(9)},${ship.vel.z.toFixed(9)}`,
    );
  ship.vel.x += deltaX;
  ship.vel.y += deltaY;
  ship.vel.z += deltaZ;
  ...
  if (DEBUG_AI) {
    try {
      console.error(
        `AI-DEBUG moveTowards postDamp preClampVel ship=${ship.vel.x.toFixed(9)},${ship.vel.y.toFixed(9)},${ship.vel.z.toFixed(9)} maxV=${maxV.toFixed(6)}`,
      );
    } catch {
      /* best-effort */
    }
 ...
 */
export function applyBoundarySteer(
  ship: Ship,
  bounds: SimBounds,
  margin: number,
  strength: number,
