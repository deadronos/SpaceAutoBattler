...
- `resolveProjectiles`/damage (`src/game/systems/damage.ts`):
  ...
  - If `aoeRadius` is set, compute affected ships within radius and apply damage (support distance falloff optionally). Maintain XP/interrupt attribution to the projectile source.
...
2. Torpedo AoE test — spawn torpedo and multiple ships inside `aoeRadius`; after detonation assert appropriate HP reductions and XP attribution.

3. Beam test — fire a `beam:laser` and assert immediate damage on the hit entity and that a beam visual is spawned for the configured TTL.

4. PD test — scenario with an incoming missile and PD turret; assert turret interrupts other tasks and focuses on the missile, destroying it when PD DPS suffices.
