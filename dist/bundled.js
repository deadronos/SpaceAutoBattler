...
// src/webgl/advancedTextureManager.ts
var AdvancedTextureManager = class {
  gl;
  anisotropyExt;
  textures = /* @__PURE__ */ new Map();
  defaultConfig;
   ...
   */
  createAdvancedTexture(key, source, config2) {
   ...
   */
  bindTextureWithLOD(key, zoomLevel = 1, textureUnit = 0) {
    const lodSet = this.textures.get(key);
    if (!lodSet) {
      return false;
    }
    ...
    if (lodSet.config.lodBias && lodSet.config.lodBias !== 0) {
      const adjustedBias = lodSet.config.lodBias + Math.log2(1 / zoomLevel);
    }
    return true;
  }
  /**
   ...
   */
  updateTextureFiltering(key, config2) {
    const lodSet = this.textures.get(key);
    if (!lodSet) {
      return false;
    }
    const gl = this.gl;
    const updatedConfig = { ...lodSet.config, ...config2 };
    gl.bindTexture(gl.TEXTURE_2D, lodSet.baseTexture);
    if (config2.magFilter !== void 0) {
      gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, config2.magFilter);
    }
    if (config2.minFilter !== void 0) {
      gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, config2.minFilter);
    }
    if (config2.wrapS !== void 0) {
      gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, config2.wrapS);
    }
    if (config2.wrapT !== void 0) {
      gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, config2.wrapT);
    }
    if (config2.useAnisotropic !== void 0 && config2.maxAnisotropy !== void 0) {
      if (config2.useAnisotropic && this.anisotropyExt.available && this.anisotropyExt.ext) {
        const anisotropy = Math.min(config2.maxAnisotropy, this.anisotropyExt.maxAnisotropy);
        gl.texParameterf(gl.TEXTURE_2D, this.anisotropyExt.ext.TEXTURE_MAX_ANISOTROPY_EXT, anisotropy);
      }
    }
  ...
  getMemoryStats() {
    ...
    return {
      ...
      details: details.sort((a, b) => b.memoryMB - a.memoryMB)
  ...
  setupTextureFiltering(gl, width, height) {
    if (this.advancedTextures) {
      const config2 = {
        ...
        useAnisotropic: true,
        maxAnisotropy: 8,
        // Higher quality for game assets
        lodBias: 0
      };
      if (config2.generateMipmaps) {
        gl.generateMipmap(gl.TEXTURE_2D);
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR_MIPMAP_LINEAR);
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.LINEAR);
      } else {
      ...
      if (config2.useAnisotropic) {
        ...
        if (anisotropyInfo.available && anisotropyInfo.ext) {
          ...
          const anisotropy = Math.min(config2.maxAnisotropy, maxAnisotropy);
