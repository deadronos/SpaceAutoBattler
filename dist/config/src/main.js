...
export async function startApp(rootDocument = document) {
    ...
    try {
        if (ui.stats)
            ui.stats.textContent = "Ships: 0 (R:0 B:0) Bullets: 0";
    }
    catch (e) { }
    ...
    function updateCanvasBackingStore() {
        ...
        if (scaleVal)
            scaleVal.textContent = renderScale.toFixed(2);
    }
    ...
    if (dynamicCheckbox) {
        ...
        dynamicCheckbox.checked = !!RendererConfig
            .dynamicScaleEnabled;
    }
    fitCanvasToWindow();
    ...
    try {
        if (typeof window !== 'undefined' && svgRenderer && typeof svgRenderer.prewarmAssets === 'function') {
            try {
                const assetsConfig = AssetsConfig || globalThis.AssetsConfig || {};
    ...
    try {
        if (typeof window !== "undefined") {
            ...
            const host = (location && location.hostname) || "";
            const urlParams = typeof URLSearchParams !== "undefined"
                ? new URLSearchParams(location.search)
                : null;
            const enabled = host === "127.0.0.1" ||
                host === "localhost" ||
    ...
    if (gm && typeof gm.stepOnce === "function") {
        ...
        gm.stepOnce = (dt = SIM.DT_MS / 1000) => origStepOnce(dt * simSpeedMultiplier);
    }
    ...
    try {
        ...
        const urlParams = typeof URLSearchParams !== "undefined"
            ? new URLSearchParams(location.search)
            : null;
        const autotest = (urlParams && urlParams.get("autotest") === "1") ||
            !!window.__AUTO_REINFORCE_DEV__;
    ...
    try {
        if (ui.reset)
            addListener(ui.reset, "click", () => gm.reset());
    }
    catch (e) { }
    ...
    try {
        if (ui.addRed)
            addListener(ui.addRed, "click", () => spawnSelected("red"));
    }
    catch (e) { }
    try {
        if (ui.addBlue)
    ...
    try {
        if (ui.seedBtn)
            addListener(ui.seedBtn, "click", onSeedBtnClick);
    }
    catch (e) { }
