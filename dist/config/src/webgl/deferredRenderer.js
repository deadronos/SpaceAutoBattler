...
export class DeferredRenderer {
     ...
     */
    createRenderTarget(name, width, height, colorFormats, depthFormat, samples = 0) {
        const gl = this.gl;
        // Create framebuffer
     ...
     */
    createColorTexture(width, height, format, samples) {
        const gl = this.gl;
     ...
     */
    createDepthTexture(width, height, format, samples) {
        const gl = this.gl;
        const texture = gl.createTexture();
        if (!texture) {
    ...
    createGBuffer(width, height) {
        ...
        const renderTarget = this.createRenderTarget('gBuffer', width, height, colorFormats, depthFormat, samples);
        const gBuffer = {
            ...renderTarget,
     ...
     */
    createPostProcessTarget(width, height, enableHDR = false) {
        const gl = this.gl;
        ...
        const renderTarget = this.createRenderTarget(`postProcess_${this.postProcessTargets.length}`, width, height, colorFormats, gl.DEPTH_COMPONENT24);
        const postProcessTarget = {
            ...renderTarget,
     ...
     */
    renderFullscreenEffect(effect, inputTextures) {
