...
export class ShadowLightingManager {
    ...
    createShadowCascades() {
        ...
        for (let i = 0; i < this.config.cascadeCount; i++) {
            ...
            gl.texImage2D(gl.TEXTURE_2D, 0, gl.DEPTH_COMPONENT24, this.config.resolution, this.config.resolution, 0, gl.DEPTH_COMPONENT, gl.UNSIGNED_INT, null);
     ...
     */
    createPointLight(position, color, intensity = 1.0, radius = 10.0, castShadows = true) {
        const light = {
    ...
    createShadowCubeMap() {
        ...
        // Create 6 faces
        for (let i = 0; i < 6; i++) {
            gl.texImage2D(gl.TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, gl.DEPTH_COMPONENT24, this.config.resolution, this.config.resolution, 0, gl.DEPTH_COMPONENT, gl.UNSIGNED_INT, null);
        }
        // Set texture parameters
     ...
     */
    createCubeMapMatrices(position, radius) {
        const matrices = [];
        ...
        for (const face of faces) {
            const view = this.createLookAtMatrix(position, [
     ...
     */
    renderShadowMaps(sceneRenderCallback) {
        const gl = this.gl;
     ...
     */
    renderDirectionalShadows(light, sceneRenderCallback) {
        const gl = this.gl;
        ...
        for (let i = 0; i < light.cascades.length; i++) {
            ...
            gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.DEPTH_ATTACHMENT, gl.TEXTURE_2D, cascade.shadowMap, 0);
            // Clear depth buffer
     ...
     */
    renderPointShadows(light, sceneRenderCallback) {
        const gl = this.gl;
        ...
        for (let face = 0; face < 6; face++) {
            ...
            gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.DEPTH_ATTACHMENT, gl.TEXTURE_CUBE_MAP_POSITIVE_X + face, light.shadowCubeMap, 0);
            // Clear depth buffer
            gl.clear(gl.DEPTH_BUFFER_BIT);
     ...
     */
    updateCascadeLightSpaceMatrix(light, cascade) {
        // Create orthographic projection for cascade
        const projection = this.createOrthographicMatrix(cascade.bounds.left, cascade.bounds.right, cascade.bounds.bottom, cascade.bounds.top, -100, // Near (extended for shadow casters)
        100 // Far
        );
        ...
        const view = this.createLookAtMatrix(lightPos, lightTarget, [0, 1, 0] // Up vector
        );
        // Combine projection and view
     ...
     */
    updatePointLightPosition(index, position) {
        if (index >= 0 && index < this.pointLights.length) {
            ...
            if (this.pointLights[index].castShadows) {
                this.pointLights[index].lightSpaceMatrices = this.createCubeMapMatrices(position, this.pointLights[index].radius);
            }
            return true;
        }
        return false;
