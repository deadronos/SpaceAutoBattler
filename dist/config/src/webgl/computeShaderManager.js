...
export class ComputeShaderManager {
     ...
     */
    createUBO(name, layout, usage = this.gl.DYNAMIC_DRAW) {
        const gl = this.gl;
     ...
     */
    createTransformFeedbackProgram(name, vertexSource, fragmentSource, varyings) {
        const gl = this.gl;
    ...
    `;
        return this.createTransformFeedbackProgram('frustumCulling', vertexSource, fragmentSource, ['v_visibility']);
    }
    /**
     * Perform frustum culling on GPU
     */
    performFrustumCulling(cullingData, viewProjectionMatrix, cameraPosition) {
        const gl = this.gl;
     ...
     */
    static extractFrustumPlanes(viewProjectionMatrix) {
        ...
        for (let i = 0; i < 6; i++) {
            const base = i * 4;
            const length = Math.sqrt(planes[base] * planes[base] +
