...
import { releaseBullet, acquireExplosion, releaseExplosion, acquireShieldHit, releaseShieldHit, acquireHealthHit, releaseHealthHit, releaseParticle, } from "./gamemanager";
import * as SpatialGridModule from "./spatialGrid";
// typed as any to avoid strict import/typing issues in this hotpath
const SpatialGrid = SpatialGridModule.default || SpatialGridModule;
const segmentIntersectsCircle = SpatialGridModule
...
export function simulateStep(state, dtSeconds, bounds) {
    ...
    function pruneAll(state, dtSeconds, bounds) {
    ...
    const fighterCountsByParent = new Map();
    try {
    ...
    for (let si = (state.ships || []).length - 1; si >= 0; si--) {
        ...
        if (remove) {
            ...
            if (rem && rem.length) {
                ...
                try {
                    if (rem[0] && rem[0].team)
                        state.teamCounts[rem[0].team] = Math.max(0, (state.teamCounts[rem[0].team] || 0) - 1);
                }
                catch (e) { }
            }
        }
                    ...
                    for (let ti = 0; ti < s.turrets.length; ti++) {
                        try {
                            ...
                            if (!t || Array.isArray(t))
                                continue; // skip tuple mounts
                            // If caller already provided a targetAngle, don't override
                            if (typeof t.targetAngle === "number")
                                continue;
                            ...
                            if (best) {
                                ...
                                const mount = Array.isArray(t.position)
                                    ? {
                                        x: (Math.cos(s.angle || 0) * t.position[0] -
                                            Math.sin(s.angle || 0) * t.position[1]) *
                                            (s.radius || 12) +
                                            (s.x || 0),
                                        y: (Math.sin(s.angle || 0) * t.position[0] +
                                            Math.cos(s.angle || 0) * t.position[1]) *
                                            (s.radius || 12) +
                                            (s.y || 0),
                                    }
                                    : { x: s.x || 0, y: s.y || 0 };
                                const desiredWorld = Math.atan2((best.y || 0) - mount.y, (best.x || 0) - mount.x);
            ...
            if (Array.isArray(s.turrets) && s.turrets.length) {
                ...
                for (let ti = 0; ti < turretDefs.length; ti++) {
                    try {
                        ...
                        try {
                            const td = AssetsConfig.turretDefaults &&
                                AssetsConfig.turretDefaults[t.kind || "basic"];
                            if (td && typeof td.turnRate === "number")
                                defaultTurn = td.turnRate;
                        }
                        catch (e) { }
                        const maxTurn = (typeof t.turnRate === "number" ? t.turnRate : defaultTurn) *
                            dtSeconds;
                ...
                if (s._carrierTimer >= cooldown) {
                    ...
                    while (toSpawn > 0) {
                        ...
                        try {
                            ...
                            try {
                                updateTeamCount(state, undefined, String(f.team));
                            }
                            catch (e) { }
                        }
                        catch (e) { }
    ...
    for (let bi = (state.bullets || []).length - 1; bi >= 0; bi--) {
        ...
        for (let ci = 0; ci < candidates.length; ci++) {
            ...
            if (!s || removedShipIds.has(s.id))
                continue;
            if (s.team === b.team)
                continue;
            const r = (s.radius || 6) + (b.radius || 1);
            // Swept collision: check segment from previous bullet pos (if available) to current pos
            const bxPrev = typeof b._prevX === "number"
                ? b._prevX
                : b.x - (b.vx || 0) * dtSeconds;
            const byPrev = typeof b._prevY === "number"
                ? b._prevY
                : b.y - (b.vy || 0) * dtSeconds;
            const didHit = dist2(b, s) <= r * r ||
            ...
            if (didHit) {
                const attacker = typeof b.ownerId === "number" || typeof b.ownerId === "string"
                    ? state.shipMap &&
                        state.shipMap.get(Number(b.ownerId))
                    : undefined;
                let dealtToShield = 0;
                let dealtToHealth = 0;
                ...
                if (s.hp <= 0) {
                    ...
                    (state.explosions ||= []).push(acquireExplosion(state, {
                        x: s.x,
                        y: s.y,
                        team: s.team,
                        life: 0.5,
                    ...
                    if (idx >= 0) {
                        ...
                        try {
                            if (s && s.team)
                                state.teamCounts[s.team] = Math.max(0, (state.teamCounts[s.team] || 0) - 1);
                        }
                        catch (e) { }
                    }
    ...
    for (const s of state.ships || []) {
        if (s.maxShield)
            s.shield = Math.min(s.maxShield, (s.shield || 0) + (s.shieldRegen || 0) * dtSeconds);
    }
