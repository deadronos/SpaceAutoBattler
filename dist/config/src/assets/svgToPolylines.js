...
function parsePathCommands(d, tolerance) {
    ...
    const pts = [];
    let curX = 0, curY = 0;
    let lastCpx = null, lastCpy = null; // for S/T shorthand
    let m;
    while ((m = cmdRe.exec(tokens))) {
        ...
        if (cmd === 'C' || cmd === 'c') {
            while (ai + 5 < args.length) {
                ...
                for (let i = 1; i < cubicPts.length; i++)
                    pts.push(cubicPts[i]);
                curX = ex;
                curY = ey;
                lastCpx = cp2x;
        ...
        if (cmd === 'S' || cmd === 's') {
            while (ai + 3 < args.length) {
                ...
                for (let i = 1; i < cubicPts2.length; i++)
                    pts.push(cubicPts2[i]);
                lastCpx = cp2x;
                lastCpy = cp2y;
                curX = ex;
                curY = ey;
...
function arcToPoints(x1, y1, x2, y2, rx, ry, angle, largeArcFlag, sweepFlag, tolerance) {
    ...
    if (!sweepFlag && deltaAng > 0)
        deltaAng -= 2 * Math.PI;
    else if (sweepFlag && deltaAng < 0)
        deltaAng += 2 * Math.PI;
...
export function svgToPolylines(svgString, options = {}) {
    ...
    for (const el of polys) {
        ...
        try {
            ...
            if (!isIdentityMatrix(mtx) && pts.length)
                applyMatrixToPoints(mtx, pts);
        }
        catch (e) { }
        pushContour(pts);
    }
