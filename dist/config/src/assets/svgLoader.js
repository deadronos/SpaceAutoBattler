import { svgToPolylines } from "./svgToPolylines";
let _svgHashes = null;
...
export function getHullOutlineFromSvg(svgText, tolerance = 1.5, assetFilename) {
...
export function parseSvgForMounts(svgText) {
    try {
        ...
        const mounts = mountEls.map((el) => {
            const slot = el.getAttribute("data-mount") ||
        ...
        const engineMounts = engineEls.map((el) => {
            const slot = el.getAttribute("data-engine-mount") ||
        ...
        const colorRegions = Array.from(svg.querySelectorAll('[data-team],[data-team-slot],[class*="team-fill-"]')).map((el) => ({
            tag: el.tagName,
            id: el.id || null,
            role: el.getAttribute("data-team") ||
...
export function applyTeamColorsToSvg(svgText, mapping, options) {
    try {
        const parser = new DOMParser();
        const doc = parser.parseFromString(svgText, "image/svg+xml");
        const svg = doc.querySelector("svg");
        ...
        for (const el of els) {
            try {
                const role = (el.getAttribute("data-team") ||
                    el.getAttribute("data-team-slot") ||
                    "").trim();
                ...
                const applyAttr = (el.getAttribute("data-team-apply") || "")
                    .trim()
                    .toLowerCase();
                const apply = applyAttr === "fill" || applyAttr === "stroke"
                    ? applyAttr
                    : applyDefault;
...
export async function rasterizeSvgToCanvasAsync(svgText, outW, outH) {
    ...
    try {
        ...
        try {
            ...
            if (c2 && canvasHasOpaquePixels(c2))
                return c2;
            if (c2)
                return c2;
        }
...
export function rasterizeSvgToCanvas(svgText, outW, outH) {
    try {
        const dataUrl = encodeSvgDataUrl(svgText);
        const img = new Image();
        img.src = dataUrl;
...
export function stripHullOnly(svgText) {
    try {
        const parser = new DOMParser();
        const doc = parser.parseFromString(svgText, "image/svg+xml");
        const svg = doc.querySelector("svg");
        ...
        for (const r of rects) {
            try {
                ...
                if (cls.includes("backdrop") || cls.includes("bg"))
                    r.remove();
                const w = parseFloat(r.getAttribute("width") || "0");
                const h = parseFloat(r.getAttribute("height") || "0");
                if (!isNaN(w) && !isNaN(h) && (w > 1000 || h > 1000))
                    r.remove();
            }
            catch (e) { }
        }
...
export async function rasterizeHullOnlySvgToCanvasAsync(svgText, outW, outH) {
...
export function rasterizeHullOnlySvgToCanvas(svgText, outW, outH) {
    try {
...
export function getCachedHullCanvasSync(svgText, outW, outH, assetKey) {
    try {
...
export async function ensureRasterizedAndCached(svgText, mapping, outW, outH, options) {
