...
export function clearTransientEvents(s) {
    ...
    try {
        ...
        if (Array.isArray(s.shieldHits))
            s.shieldHits.length = 0;
        if (Array.isArray(s.healthHits))
            s.healthHits.length = 0;
    }
    catch (e) { /* ignore */ }
}
function tick() {
    ...
    while (acc >= simDtMs) {
        try {
            ...
            applySimpleAI(state, simDtMs / 1000, bounds);
