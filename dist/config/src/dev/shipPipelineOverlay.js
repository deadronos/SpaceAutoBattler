...
function detectPipeline(type) {
    ...
    if (sprite.svg)
        return { pipeline: "svg", source: sprite.svg };
    if (sprite.shape)
        return { pipeline: "shape2d", source: "shapes2d" };
...
function createOverlay() {
    ...
    const urlParams = typeof URLSearchParams !== "undefined"
        ...
        : null;
    const enabled = urlParams?.get("devShipTable") === "1" ||
        host === "127.0.0.1" ||
        host === "localhost";
    if (!enabled)
        return null;
    ...
    // restore state
    try {
        if (localStorage.getItem("shipPipelineOpen") === "1")
            setOpen(true);
    }
    catch (e) { }
