export default class TintedHullPool {
    map = new Map();
    teamMap = new Map(); // teamColor -> array of keys (in insertion order)
    ...
    set(key, canvas) {
        ...
        const nodeEnv = (typeof process !== 'undefined' && (process.env && process.env.NODE_ENV)) ? process.env.NODE_ENV : (typeof globalThis.NODE_ENV !== 'undefined' ? globalThis.NODE_ENV : 'development');
        const throwFlag = (typeof process !== 'undefined' && process.env && process.env.THROW_ON_SHARED_TINT) ? process.env.THROW_ON_SHARED_TINT : (typeof globalThis.THROW_ON_SHARED_TINT !== 'undefined' ? globalThis.THROW_ON_SHARED_TINT : undefined);
        ...
        if (!this.teamMap.has(team))
            this.teamMap.set(team, []);
