...
async function run() {
    ...
    const keys = await page.evaluate(() => {
        try {
            ...
            return { keys: Array.from((r._tintedHullCache && r._tintedHullCache.keys && r._tintedHullCache.keys()) || []) };
        }
        catch (e) {
            ...
            try {
                const data = await page.evaluate((key) => {
                    try {
                        const r = window.__renderer;
                        if (!r)
                            return { key, error: 'no_renderer' };
                        const c = r._tintedHullCache.get(key);
                        if (!c)
                            return { key, error: 'no_canvas' };
                        try {
                ...
                console.log('failed to dump', k, e);
            }
        }
    }
    await browser.close();
