...
async function run() {
    ...
    const results = await page.evaluate(async (svgBase64) => {
        ...
        try {
            ...
            if (!hullDataUrl)
                diagnostics.push('rasterize returned null for hull');
        }
        catch (e) {
        ...
        try {
            ...
            if (!multiplyDataUrl)
                diagnostics.push('multiply tint returned null');
        }
        catch (e) {
            diagnostics.push('multiply tint threw: ' + (e && e.message));
        }
        try {
            atopDataUrl = await tintSourceAtop(hullSvg, teamColor);
            if (!atopDataUrl)
                diagnostics.push('source-atop tint returned null');
        }
        catch (e) {
            diagnostics.push('source-atop tint threw: ' + (e && e.message));
        }
        return {
            hull: hullDataUrl,
    ...
    try {
        console.log('page diagnostics:', results.diagnostics || []);
    }
    catch (e) { }
    try {
