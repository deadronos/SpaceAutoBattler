...
async function run() {
    ...
    if (hasGm) {
        ...
        try {
            ...
            await page.evaluate(() => {
                try {
                    window.gm.setContinuousEnabled && window.gm.setContinuousEnabled(true);
                }
                catch (e) {
                    console.error('gm.setContinuousEnabled error', String(e));
                }
                try {
                    window.gm.setReinforcementInterval && window.gm.setReinforcementInterval(0.01);
                }
                catch (e) {
                    console.error('gm.setReinforcementInterval error', String(e));
                }
                try {
                    window.gm.stepOnce && window.gm.stepOnce(0.02);
                }
                catch (e) {
                    console.error('gm.stepOnce error', String(e));
                }
            });
            await page.waitForTimeout(300);
            const statsAfter = await page.evaluate(() => { const el = document.querySelector('#stats'); return el ? el.innerText : null; });
            const lastReinf = await page.evaluate(() => { try {
                return window.gm && typeof window.gm.getLastReinforcement === 'function' ? window.gm.getLastReinforcement() : null;
            }
            catch (e) {
                return { error: String(e) };
            } });
            ...
            console.log('gm invocation error', e);
        }
    }
    await browser.close();
}
