...
test.describe('OffscreenCanvas SVG Rasterization', () => {
    ...
    `;
        ...
        const result = await page.evaluate(async (svgText) => {
            ...
            async function testRasterization(svg) {
                try {
    ...
    `;
        const result = await page.evaluate(async (svgText) => {
            ...
            function applySvgTeamColorTinting(svg, teamColor) {
                return svg.replace(/id="hull"[^>]*>/g, (match) => match.replace(/fill="[^"]*"/, `fill="${teamColor}"`));
            }
    ...
    `;
        const result = await page.evaluate(async (svgText) => {
            try {
                ...
                const ctx = canvas.getContext('2d');
                if (!ctx)
                    return { success: false, error: 'No 2D context' };
