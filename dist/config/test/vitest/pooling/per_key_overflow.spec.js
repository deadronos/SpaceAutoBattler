...
describe('pooling per-key overflow strategies (sprites)', () => {
    ...
    it('discard-oldest trims oldest entries and calls disposer (sprites)', () => {
        ...
        releaseSprite(state, key, s1, (s) => { s.alive = false; });
        releaseSprite(state, key, s2, (s) => { s.alive = false; });
        releaseSprite(state, key, s3, (s) => { s.alive = false; });
    ...
    it('grow strategy allows exceeding max (sprites)', () => {
        ...
        releaseSprite(state, key, a, (s) => { s.alive = false; });
        releaseSprite(state, key, b, (s) => { s.alive = false; });
        const entry = state.assetPool.sprites.get(key);
        expect(entry).toBeTruthy();
        expect(entry.freeList.length).toBe(2);
    });
...
describe('pooling per-key overflow strategies (effects)', () => {
    ...
    it('discard-oldest trims oldest entries and calls disposer (effects)', () => {
        ...
        releaseEffect(state, key, e1, (e) => { e.alive = false; });
        releaseEffect(state, key, e2, (e) => { e.alive = false; });
        releaseEffect(state, key, e3, (e) => { e.alive = false; });
    ...
    it('grow strategy allows exceeding max (effects)', () => {
        ...
        releaseEffect(state, key, a, (e) => { e.alive = false; });
        releaseEffect(state, key, b, (e) => { e.alive = false; });
        const entry = state.assetPool.effects.get(key);
        expect(entry).toBeTruthy();
        expect(entry.freeList.length).toBe(2);
    });
...
describe('pooling per-key overflow strategies', () => {
    ...
    it('discard-oldest trims oldest entries and calls disposer (textures)', () => {
        ...
        releaseTexture(state, key, t1, (t) => gl.deleteTexture(t));
        releaseTexture(state, key, t2, (t) => gl.deleteTexture(t));
        releaseTexture(state, key, t3, (t) => gl.deleteTexture(t));
    ...
    it('grow strategy allows exceeding max (textures)', () => {
        ...
        releaseTexture(state, key, a, (t) => gl.deleteTexture(t));
        releaseTexture(state, key, b, (t) => gl.deleteTexture(t));
        const entry = state.assetPool.textures.get(key);
        expect(entry).toBeTruthy();
