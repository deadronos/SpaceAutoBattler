...
import { acquireBullet, releaseBullet, acquireExplosion, releaseExplosion, acquireShieldHit, releaseShieldHit, acquireHealthHit, releaseHealthHit, acquireParticle, releaseParticle, reset, } from "../../src/gamemanager";
...
describe("Object Pooling", () => {
    ...
    it("recycles all expired bullets (leak detection)", () => {
        ...
        expect((state.bullets || []).every((b) => !b.alive)).toBe(true);
    });
    ...
    it("releases all expired explosions (missed object detection)", () => {
        ...
        expect((state.explosions || []).every((f) => !f.alive)).toBe(true);
    });
