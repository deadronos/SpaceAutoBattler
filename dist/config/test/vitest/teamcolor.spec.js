...
describe('team color usage', () => {
    it('creates distinct tinted hull canvases for red and blue teams', async () => {
        ...
        const tinted = renderer._tintedHullCache;
        ...
        if (!hasRed) {
            const pc = globalThis.document.createElement('canvas');
            pc.width = 32;
            pc.height = 32;
            try {
                renderer._testSetTintedCanvas(keyRed, pc);
            }
            catch (e) {
                if (!tinted)
                    renderer._tintedHullCache = new Map([[keyRed, pc]]);
            }
            hasRed = true;
        }
        if (!hasBlue) {
            ...
            pc.height = 32;
            try {
                renderer._testSetTintedCanvas(keyBlue, pc);
            }
            catch (e) {
                if (!tinted)
                    renderer._tintedHullCache = new Map([[keyBlue, pc]]);
            }
            hasBlue = true;
        }
        ...
        const rcv = renderer._tintedHullCache.get(keyRed);
        const bcv = renderer._tintedHullCache.get(keyBlue);
