...
describe('turret simulation', () => {
    it('advances turret.angle toward targetAngle by at most turnRate * dt', () => {
        ...
        const attacker = { id: 1, x: 50, y: 50, angle: 0, radius: 12, hp: 10, maxHp: 10, team: 'red', turrets: [{ position: [1, 0], angle: 0, targetAngle: Math.PI / 2, turnRate: Math.PI }] };
        const defender = { id: 2, x: 60, y: 60, angle: 0, radius: 12, hp: 10, maxHp: 10, team: 'blue' };
        state.ships = [attacker, defender];
        // step with dt = 0.5s, max rotation = PI * 0.5 = 1.5708
        simulateStep(state, 0.5, { W: 200, H: 200 });
        const t = state.ships[0].turrets[0];
    ...
    it('basic AI sets turret.targetAngle toward nearest enemy', () => {
        const state = makeInitialState();
        const attacker = { id: 1, x: 10, y: 10, angle: 0, radius: 12, hp: 10, maxHp: 10, team: 'red', turrets: [{ position: [1, 0], angle: 0 }] };
        const enemy = { id: 2, x: 20, y: 10, angle: 0, radius: 12, hp: 10, maxHp: 10, team: 'blue' };
