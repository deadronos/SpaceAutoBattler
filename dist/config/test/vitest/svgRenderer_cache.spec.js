...
describe('rasterizeSvgWithTeamColors cache', () => {
    it('returns the same canvas for repeated calls with same key and mapping', async () => {
        ...
        const a = rasterizeSvgWithTeamColors(sampleSvg, mapping, 10, 10, { assetKey: 'asset-1' });
        const b = rasterizeSvgWithTeamColors(sampleSvg, mapping, 10, 10, { assetKey: 'asset-1' });
    ...
    it('clearing cache causes new promise to be created', async () => {
        _clearRasterCache();
        const mapping = { primary: '#112233' };
        const a = rasterizeSvgWithTeamColors(sampleSvg, mapping, 10, 10, { assetKey: 'asset-2' });
        const first = await a;
        _clearRasterCache();
        const b = rasterizeSvgWithTeamColors(sampleSvg, mapping, 10, 10, { assetKey: 'asset-2' });
