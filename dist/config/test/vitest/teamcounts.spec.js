...
describe('teamCounts', () => {
    it('increments on spawnShip and decrements after removal (main thread)', () => {
        ...
        const actualRed = (snap1.ships || []).filter((sh) => sh.team === 'red').length;
        ...
        if (typeof gm.stepOnce === 'function')
            gm.stepOnce(0.1);
        const snap2 = gm.snapshot();
        const actualRed2 = (snap2.ships || []).filter((sh) => sh.team === 'red').length;
    ...
    it('rebuilds teamCounts when worker snapshot/setState is applied', () => {
        ...
        const actualRed = (snap.ships || []).filter((sh) => sh.team === 'red').length;
        const actualBlue = (snap.ships || []).filter((sh) => sh.team === 'blue').length;
