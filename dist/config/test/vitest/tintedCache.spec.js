...
describe('CanvasRenderer tinted hull cache', () => {
    ...
    beforeEach(() => {
        ...
        AssetsConfig.svgAssets = AssetsConfig.svgAssets || {};
        AssetsConfig.svgAssets.testfighter = '<svg viewBox="0 0 128 128"><rect x="0" y="0" width="128" height="128" fill="#fff" /></svg>';
        // Add a shapes2d entry to avoid extent computation issues
        AssetsConfig.shapes2d = AssetsConfig.shapes2d || {};
        AssetsConfig.shapes2d.testfighter = { type: 'circle', r: 10 };
        // Provide a minimal team config
        TeamsConfig.teams = { red: { id: 'red', color: '#ff0000' }, blue: { id: 'blue', color: '#0000ff' } };
    });
    it('pre-warms tinted cache for known teams', async () => {
        ...
        r._tintedHullPool = new TintedHullPool({ globalCap: 8, perTeamCap: 4 });
        await r.preloadAllAssets();
        // After preload, expect tinted pool to contain entries for the testfighter+teams
        const keys = r._tintedHullPool ? Array.from(r._tintedHullPool.keys()) : [];
        // diagnostic checks
        const declared = AssetsConfig.svgAssets || {};
        // DEBUG: dump declared assets and renderer caches
        console.log('[test] declared svgAssets keys=', Object.keys(declared));
        const hulls = r._svgHullCache || {};
    ...
    it('evicts LRU entries when cap exceeded and promotes on access', async () => {
        ...
        r._tintedHullPool = new TintedHullPool({ globalCap: 3, perTeamCap: 3 });
        ...
        c4.width = c4.height = 32;
        r._svgHullCache = { a: c1, b: c2, c: c3, d: c4 };
    ...
    it('clearTintedHullCache clears the cache', () => {
        const r = new CanvasRenderer(canvas);
        r._tintedHullPool = new TintedHullPool({ globalCap: 10, perTeamCap: 5 });
        r._tintedHullPool.set('x::#1', document.createElement('canvas'));
        r.clearTintedHullCache();
        expect(r._tintedHullPool.size).toBe(0);
    });
});
