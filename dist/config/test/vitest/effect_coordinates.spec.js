...
class Stub2DContext {
    ...
    restore() { const s = this.stack.pop(); if (s) {
        this.a = s.a;
        this.b = s.b;
        this.c = s.c;
        this.d = s.d;
    ...
    drawImage() { }
    fillRect() { }
    moveTo(x, y) { const tx = this.a * x + this.c * y + this.e; const ty = this.b * x + this.d * y + this.f; this.pathPoints.push({ x: tx, y: ty }); }
    lineTo(x, y) { const tx = this.a * x + this.c * y + this.e; const ty = this.b * x + this.d * y + this.f; this.pathPoints.push({ x: tx, y: ty }); }
    closePath() { }
}
describe('Effect coordinate rendering', () => {
    it('explosion is drawn at world coordinates even after ship draw (no transform leakage)', () => {
        ...
        state.explosions = [{ x: 200, y: 200, r: 0.32, life: 0.5, ttl: 0.5 }];
        // Render
    ...
    it('shield ring is drawn at ship-local center (0,0) but maps to world coords', () => {
        ...
        state.ships.push(ship);
        renderer.renderState(state);
        const found = stub2.drawnArcs.find((a) => Math.abs(a.x - 300) < 1e-6 && Math.abs(a.y - 300) < 1e-6);
    ...
    it('health flash is drawn at the provided world coordinates', () => {
        ...
        renderer.bufferCtx = stub3;
        const state = makeInitialState();
        const ship = createShip('fighter', 400, 400, 'red');
        state.ships.push(ship);
        // Add a health flash at (410, 410)
        state.healthFlashes = [{ id: ship.id, x: 410, y: 410, _ts: state.t || 0, ttl: 0.4 }];
        renderer.renderState(state);
        const found = stub3.drawnArcs.find((a) => Math.abs(a.x - 410) < 1e-6 && Math.abs(a.y - 410) < 1e-6);
    ...
    it('engine flare polygon is drawn at the expected engine offset in world coords', () => {
        ...
        const engineCfg = (visual && visual.animations && visual.animations.engineFlare) || Assets.animations.engineFlare;
    ...
    it('turret is drawn at computed turret world position', () => {
        ...
        let shipCfgForTurrets = cfgAll['destroyer'];
        ...
        if (Array.isArray(shipCfgForTurrets.turrets)) {
            ship.turrets = shipCfgForTurrets.turrets.map((t) => ({ position: t.position, kind: t.kind || 'basic' }));
        }
        state.ships.push(ship);
        renderer.renderState(state);
        ...
        function translateMat(tx, ty) { return [1, 0, 0, 1, tx, ty]; }
        function rotateMat(v) { const c = Math.cos(v); const s = Math.sin(v); return [c, s, -s, c, 0, 0]; }
        function scaleMat(sx, sy) { return [sx, 0, 0, sy, 0, 0]; }
        ...
        let shipCfg = getShipConfigSafe()['destroyer'];
        if (!shipCfg) {
        ...
        if (turretShape.type === 'polygon')
            polyPoints = turretShape.points || null;
        else if (turretShape.type === 'compound') {
            const part = turretShape.parts?.find((p) => p.type === 'polygon');
        ...
        const centerFromPath = stub.pathPoints.find((p) => Math.hypot(p.x - turretCenter.x, p.y - turretCenter.y) <= centerApproxTol + 1e-6);
        ...
        let farthest = null;
        let farthestDist = -1;
        for (const t of stub.translateHistory) {
        ...
        expect(centerFromPath || directionMatches, `Expected turret geometry near center (${turretCenter.x}, ${turretCenter.y}) or translated in correct direction.\n` +
            `pathPoints: ${JSON.stringify(stub.pathPoints.slice(0, 40))}\n` +
            `translates: ${JSON.stringify(stub.translateHistory.slice(0, 40))}`).toBeTruthy();
    });
});
