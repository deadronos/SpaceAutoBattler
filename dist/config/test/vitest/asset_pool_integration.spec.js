...
describe('asset pool integration (texture/sprite/effect)', () => {
    ...
    it('effects pooling respects config and reset semantics', () => {
        ...
        const factory = createPooledFactory(() => ({ x: 0, alive: true }), (o, init) => { o.x = init?.x ?? 0; o.alive = true; });
        const objs = [];
        for (let i = 0; i < 4; ++i)
            objs.push(acquireEffect(state, 'fx', () => makePooled(factory.create(), factory.reset), { x: i }));
        for (const o of objs)
            releaseEffect(state, 'fx', o);
