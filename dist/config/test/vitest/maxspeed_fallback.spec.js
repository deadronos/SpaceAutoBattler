...
test('createShip falls back to positive maxSpeed when config is missing or zero', () => {
    ...
    try {
        ...
        if (entitiesConfig.ShipConfig && entitiesConfig.ShipConfig.fighter) {
            entitiesConfig.ShipConfig.fighter.maxSpeed = 0;
        }
        ...
        try {
            entitiesConfig.ShipConfig = orig.ShipConfig || orig;
        }
        catch (e) { }
    }
});
