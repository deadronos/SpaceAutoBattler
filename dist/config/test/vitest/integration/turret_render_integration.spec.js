...
function computeRendererTurretWorldCoord(ship, turretPosTuple) {
...
describe('integration: turret render vs bullet spawn', () => {
    it('renderer turret world coord equals bullet spawn coord for tuple turret', () => {
        ...
        const attacker = { id: 100, x: 100, y: 100, angle: 0, radius: 10, type: 'unknown-type', hp: 10, maxHp: 10, team: 'red', turrets: [[1, 0]] };
        const defender = { id: 101, x: 140, y: 100, angle: 0, radius: 12, hp: 10, maxHp: 10, team: 'blue' };
    ...
    it('renderer turret world coord equals bullet spawn coord for rotated ship', () => {
        ...
        const attacker = { id: 200, x: 200, y: 200, angle, radius: 8, type: 'unknown-type', hp: 10, maxHp: 10, team: 'red', turrets: [[1, 0]] };
        const defender = { id: 201, x: 200, y: 240, angle: 0, radius: 12, hp: 10, maxHp: 10, team: 'blue' };
        state.ships = [attacker, defender];
        applySimpleAI(state, 1.0, getDefaultBounds());
        simulateStep(state, 0, getDefaultBounds());
        expect(Array.isArray(state.bullets)).toBe(true);
    ...
    it('multiple tuple turrets spawn bullets at their respective mountpoints', () => {
        ...
        const attacker = { id: 300, x: 300, y: 300, angle: 0, radius: 10, type: 'unknown-type', hp: 10, maxHp: 10, team: 'red', turrets: [[1, 0], [-1, 0]] };
        const defender = { id: 301, x: 340, y: 300, angle: 0, radius: 12, hp: 10, maxHp: 10, team: 'blue' };
        state.ships = [attacker, defender];
        applySimpleAI(state, 1.0, getDefaultBounds());
        simulateStep(state, 0, getDefaultBounds());
        expect(Array.isArray(state.bullets)).toBe(true);
    ...
    it('object-style turret kinds and mixed tuple/object entries spawn at correct coords', () => {
        ...
        const defender = { id: 401, x: 440, y: 140, angle: 0, radius: 12, hp: 10, maxHp: 10, team: 'blue' };
        state.ships = [attacker, defender];
        applySimpleAI(state, 1.0, getDefaultBounds());
        simulateStep(state, 0, getDefaultBounds());
        expect(Array.isArray(state.bullets)).toBe(true);
    ...
    it('carrier turret mountpoint scales with carrier radius and spawns bullet at correct coord', () => {
        ...
        const turretPos = carrierCfg && Array.isArray(carrierCfg.turrets) && carrierCfg.turrets.length ? carrierCfg.turrets[0].position : [2.0, 1.2];
        const attacker = { id: 500, x: 500, y: 100, angle: 0, radius: carrierCfg.radius || 40, type: 'carrier', hp: 10, maxHp: 10, team: 'red', turrets: [turretPos] };
        const defender = { id: 501, x: 560, y: 100, angle: 0, radius: 12, hp: 10, maxHp: 10, team: 'blue' };
        state.ships = [attacker, defender];
    ...
    it('negative rotation turret spawn matches renderer math', () => {
        ...
        const attacker = { id: 600, x: 600, y: 200, angle, radius: 9, type: 'unknown-type', hp: 10, maxHp: 10, team: 'red', turrets: [[0.5, -0.5]] };
        const defender = { id: 601, x: 640, y: 160, angle: 0, radius: 12, hp: 10, maxHp: 10, team: 'blue' };
        state.ships = [attacker, defender];
        applySimpleAI(state, 1.0, { W: 1200, H: 800 });
        simulateStep(state, 0, { W: 1200, H: 800 });
        expect(Array.isArray(state.bullets)).toBe(true);
    ...
    it('fighters spawned by carrier have normalized turret objects', () => {
        ...
        const carrier = { id: 700, x: 700, y: 300, angle: 0, radius: carrierCfg.radius || 40, type: 'carrier', hp: 100, maxHp: 100, team: 'red', turrets: (carrierCfg.turrets || []).map((t) => t.position) };
        ...
        const spawned = (state.ships || []).filter((s) => s && s.parentId === carrier.id && s.type === 'fighter');
