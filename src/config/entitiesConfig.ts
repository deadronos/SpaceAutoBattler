...
export const ShipConfig: ShipConfigMap = {
  fighter: {
    maxHp: 15, armor: 0, maxShield: 8, shieldRegen: 1.0, dmg: 3, damage: 3, radius: 12,
    cannons: [ { damage: 3, rate: 3, spread: 0.1, muzzleSpeed: 300, bulletRadius: 1.5, bulletTTL: 1.2 } ],
    accel: 150, turnRate: 6,
  },
  corvette: {
    maxHp: 50, armor: 0, maxShield: Math.round(50 * 0.6), shieldRegen: 0.5, dmg: 5, damage: 5, radius: 20,
    accel: 80, turnRate: 3,
    cannons: [ { damage: 6, rate: 1.2, spread: 0.05, muzzleSpeed: 220, bulletRadius: 2, bulletTTL: 2.0 } ],
  },
  frigate: {
    maxHp: 80, armor: 1, maxShield: Math.round(80 * 0.6), shieldRegen: 0.4, dmg: 8, damage: 8, radius: 24,
    cannons: [ { damage: 8, rate: 1.0, spread: 0.06, muzzleSpeed: 200, bulletRadius: 2.5, bulletTTL: 2.2 } ],
    accel: 60, turnRate: 2.2,
  },
  destroyer: {
    maxHp: 120, armor: 2, maxShield: Math.round(120 * 0.6), shieldRegen: 0.3, dmg: 12, damage: 12, radius: 40,
    cannons: new Array(6).fill(0).map(() => ({ damage: 6, rate: 0.8, spread: 0.08, muzzleSpeed: 240, bulletRadius: 2.5, bulletTTL: 2.4 })),
    accel: 40, turnRate: 1.6,
    turrets: [
  ...
  carrier: {
    maxHp: 200, armor: 3, maxShield: Math.round(200 * 0.6), shieldRegen: 0.2, dmg: 2, damage: 2, radius: 40,
    cannons: new Array(4).fill(0).map(() => ({ damage: 4, rate: 0.6, spread: 0.12, muzzleSpeed: 180, bulletRadius: 3, bulletTTL: 2.8 })),
    accel: 20, turnRate: 0.8,
    carrier: { fighterCooldown: 1.5, maxFighters: 6, spawnPerCooldown: 2 },
    turrets: [
      { position: [2.0, 1.2], kind: 'basic', targeting: 'nearest', cooldown: 1.0 },
      { position: [-2.0, 1.2], kind: 'basic', targeting: 'nearest', cooldown: 1.0 },
      { position: [2.0, -1.2], kind: 'basic', targeting: 'nearest', cooldown: 1.0 },
      { position: [-2.0, -1.2], kind: 'basic', targeting: 'nearest', cooldown: 1.0 }
    ]
