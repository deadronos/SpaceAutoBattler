...
export function assignRoamingAnchor(state: GameState, ship: Ship): Vector3 {
  ...
  const maybeBounds = (state as unknown as { bounds?: { width:number; height:number; depth:number }; worldBounds?: { width:number; height:number; depth:number } });
  const bounds = maybeBounds.bounds ?? maybeBounds.worldBounds ?? { width: 1000, height: 1000, depth: 1000 };
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    for (const a of anchors) {
      const dx = a.pos.x - candidate.x; const dy = a.pos.y - candidate.y; const dz = a.pos.z - candidate.z;
      const dSq = dx*dx + dy*dy + dz*dz;
      if (dSq < minSep * minSep) { ok = false; break; }
    }
    if (ok) {
