...
export function acquireEffect<T extends object>(
  ...
  const poolMap = state.assetPool.effects as Map<
    string,
    PoolEntry<T & Pooled<T>>
  >;
...
export function acquireTexture(
  ...
  const max =
    (entry.config?.max ?? state.assetPool.config.texturePoolSize) || 128;
  const strategy =
...
export function releaseTexture(
  ...
  const countsMap =
    state.assetPool.counts?.textures || new Map<string, number>();
  if (strategy === "grow") return;
  while (free.length > max) {
    const victim = strategy === "discard-oldest" ? free.shift()! : free.pop()!;
    try {
...
export function acquireSprite<T extends object>(
  ...
  const poolMap = state.assetPool.sprites as Map<
    string,
    PoolEntry<T & Pooled<T>>
  >;
