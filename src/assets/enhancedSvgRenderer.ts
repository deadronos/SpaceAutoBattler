...
import { AtlasManager, AtlasEntry } from './textureAtlas';

export interface EnhancedRasterOptions {
  teamColor?: string;
  useWorker?: boolean;
  useCache?: boolean;
  useAtlas?: boolean;
 ...
 */
export async function enhancedRasterizeSvgWithTeamColors(
  svgText: string,
  teamColors: { hull?: string; accent?: string },
  options: EnhancedRasterOptions = {}
  ...
  if (options.useAtlas && options.atlasManager && options.assetKey) {
    try {
      const atlasEntry = await addSvgToAtlas(
        svgText,
        teamColors,
 ...
 */
async function basicCanvasRasterization(
  svgText: string,
  teamColors: { hull?: string; accent?: string },
  size: number
): Promise<HTMLCanvasElement> {
  ...
  if (teamColors.hull) {
    processedSvg = processedSvg.replace(/id="hull"[^>]*>/g, (match) => 
      match.replace(/fill="[^"]*"/, `fill="${teamColors.hull}"`)
    );
  }
  if (teamColors.accent) {
    processedSvg = processedSvg.replace(/id="accent"[^>]*>/g, (match) => 
      match.replace(/fill="[^"]*"/, `fill="${teamColors.accent}"`)
    );
  }
 ...
 */
async function enhancedRasterizeSvgWithoutAtlas(
  svgText: string,
  teamColors: { hull?: string; accent?: string },
  options: Omit<EnhancedRasterOptions, 'useAtlas' | 'atlasManager' | 'assetKey'>
): Promise<HTMLCanvasElement> {
  const size = options.size || 128;
  const teamColor = teamColors.hull;
 ...
 */
function createCanvasFromAtlasEntry(
  entry: AtlasEntry,
  atlasManager: AtlasManager
): HTMLCanvasElement {
  const atlas = atlasManager.getAtlas('main');
  ...
  if (ctx) {
    ...
    (canvas as any).atlasEntry = entry;
    (canvas as any).isAtlasBacked = true;
  }
 ...
 */
export function getAtlasEntryFromCanvas(canvas: HTMLCanvasElement): AtlasEntry | null {
  return (canvas as any).atlasEntry || null;
}

/**
 * Check if a canvas is atlas-backed
 */
export function isAtlasBacked(canvas: HTMLCanvasElement): boolean {
  return !!(canvas as any).isAtlasBacked;
}

/**
 * Batch process multiple SVGs into atlas
 */
export async function batchAddSvgsToAtlas(
  svgEntries: Array<{
    svgText: string;
    teamColors: { hull?: string; accent?: string };
    assetKey: string;
  ...
  atlasManager: AtlasManager
): Promise<Array<AtlasEntry | null>> {
  const results: Array<AtlasEntry | null> = [];
