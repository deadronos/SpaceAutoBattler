import { Vector3 } from 'three';
import type { ShipStats } from '../../types/index.js';
import { HULL_DAMAGE_TYPES, HULL_ARMOR_VALUES } from '../../config/progression.js';
import {
  createIonTurret,
  createLaserTurret,
  createMissileTurret,
  createBeamTurret,
} from './turret-factory.js';

export const CARRIER_STATS: ShipStats = {
  hull: 'carrier',
  maxHp: 320,
  maxShield: 255,
  shieldRegen: 13.0,
  damage: 28,
  fireRate: 2.2,
  projectileSpeed: 67,
  range: 600,
  speed: 7,
  scale: 1,
  bulletType: 'bullet:ion',
  damageType: HULL_DAMAGE_TYPES.carrier,
  armor: HULL_ARMOR_VALUES.carrier,
  motion: {
    mass: 6.0,
    maxSpeed: 7,
    maxReverseSpeed: 2,
    linearAcceleration: 6,
    linearDamping: 1.0,
    maxTurnRate: Math.PI * 0.4,
    angularAcceleration: Math.PI * 1.2,
    angularDamping: 3.0,
    turnKp: 3.0,
    turnKd: 0.9,
    angularSettlingRate: 0.007,
    angularSettleToleranceDeg: 5,
    maxLateralAcceleration: 2,
    visualBankFactor: 8,
    maxBankDeg: 18,
    visual: {
      enabled: true,
      position: { k: 8.0 },
      rotation: { k: 16.0 },
      bank: { k: 12.0, maxDeg: 18, useCriticallyDamped: true },
      teleportDistance: 45,
      bob: { enabled: false, baseAmp: 0.04, freq: 0.8, speedScale: 0.8, maxAmp: 0.18 },
    },
  },
  sensor: {
    detectionRange: 900,
    trackingRange: 1100,
    coneAngle: Math.PI * 0.55,
    falloff: 0.75,
  },
  stealth: 0,
  sensorSignature: 1.8,
  turrets: [
    createIonTurret(new Vector3(2.0, 0.35, 0.0)),
    createIonTurret(new Vector3(-2.0, 0.35, 0.0)),
    createLaserTurret(new Vector3(0.0, 0.35, 1.2), {
      damage: 9,
      fireRate: 1.5,
      range: 460,
      minYaw: -Math.PI * 0.7,
      maxYaw: Math.PI * 0.7,
      minPitch: -Math.PI * 0.25,
      maxPitch: Math.PI * 0.5,
    }),
    createLaserTurret(new Vector3(0.0, 0.35, -1.2), {
      damage: 9,
      fireRate: 1.5,
      range: 460,
      minYaw: -Math.PI * 0.7,
      maxYaw: Math.PI * 0.7,
      minPitch: -Math.PI * 0.25,
      maxPitch: Math.PI * 0.5,
    }),
    createLaserTurret(new Vector3(2.4, 0.45, 1.45), {
      damage: 9,
      fireRate: 1.5,
      projectileSpeed: 250,
      range: 460,
      minYaw: -Math.PI * 0.75,
      maxYaw: Math.PI * 0.75,
      minPitch: -Math.PI * 0.35,
      maxPitch: Math.PI * 0.55,
    }),
    createLaserTurret(new Vector3(-2.4, 0.45, 1.45), {
      damage: 9,
      fireRate: 1.5,
      projectileSpeed: 250,
      range: 460,
      minYaw: -Math.PI * 0.75,
      maxYaw: Math.PI * 0.75,
      minPitch: -Math.PI * 0.35,
      maxPitch: Math.PI * 0.55,
    }),
    createMissileTurret(new Vector3(0, 0.4, -1.8), {
      damage: 22,
      fireRate: 2.6,
      projectileSpeed: 94,
      range: 660,
      minYaw: -Math.PI * 0.35,
      maxYaw: Math.PI * 0.35,
      minPitch: -Math.PI * 0.25,
      maxPitch: Math.PI * 0.35,
    }),
    createBeamTurret(new Vector3(0, 0.42, 1.8), {
      damage: 18,
      fireRate: 2.5,
      range: 600,
      minYaw: -Math.PI * 0.5,
      maxYaw: Math.PI * 0.5,
      minPitch: -Math.PI * 0.3,
      maxPitch: Math.PI * 0.5,
    }),
  ],
};
