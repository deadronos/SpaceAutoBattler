...
const resolveTexture = (texture: Texture | null | undefined, fallback: Texture): Texture =>
  texture ?? fallback;
...
export function createMainSequenceStarMaterial(
  ...
  const _enableShaderDevLogging =
    process.env.NODE_ENV !== 'production' || /[?&]copilot_debug=1/.test(_urlSearch);
    ...
    try {
      material.onBeforeCompile = (shader, renderer) => {
        ...
        try {
          ...
          console.log(
            '[STARDEV] vertex shader (trunc):',
            (shader.vertexShader || '').slice(0, 1024),
          );
          console.log(
        ...
        try {
          ...
          const props = (
            renderer as unknown as { properties?: { get: (k: ShaderMaterial) => unknown } }
          ).properties;
        ...
        for (const k of Object.keys(u)) {
          ...
          if (v == null) {
            ...
            if (maybeArrProvider && typeof maybeArrProvider.toArray === 'function') {
              try {
                uniformsMap[k] = (maybeArrProvider.toArray as Function).call(v);
              } catch {
              ...
              uniformsMap[k] = {
                ...
                generateMipmaps: Boolean(
                  (tex as unknown as { generateMipmaps?: boolean }).generateMipmaps,
                ),
