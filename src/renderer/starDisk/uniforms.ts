...
export function deriveBoundaryUniform(
  ...
  const start = clamp(rawStart, 0.6, 0.999);
...
export function deriveHazeUniform(
  ...
  const threshold = clamp(rawThreshold, 0, 0.9);
  ...
  const fade = clamp(horizonFloor + (1 - horizonFloor) * normalized, 0, 1.1);
