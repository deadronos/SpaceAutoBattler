...
// 2) node tools/renderDot.cjs docs/flowchart.dot docs/flowchart.svg
...
async function main() {
  ...
  if (args.length < 2) {
    console.error('Usage: node tools/renderDot.cjs input.dot output.svg');
    process.exit(2);
  }
  ...
  try {
    ...
    console.error('Then rerun: node tools/renderDot.cjs', inPath, outPath);
    process.exit(3);
  }

  const { Module, render } = renderModule;
  const VizClass = Viz;
  const viz = new VizClass({ Module, render });
